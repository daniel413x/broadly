...
const seed = async () => {
  ...
  try {
    ...
    // create performance testing products tenant
    const jmeterShopTenant = await payload.create({
      collection: "tenants",
      data: {
        name: "jmeter-shop",
        slug: "jmeter-shop",
        stripeAccountId: "acct_1RnC0UIPsLLJOPYt",
    ...
    await payload.create({
      collection: "users",
      data: {
        email: "jmeter-shop@jmeter-shop.com",
        username: "jmeter-shop",
        password: "jmeter-shop",
        roles: ["user"],
        tenants: [
          {
            tenant: jmeterShopTenant.id,
          },
        ],
      },
      req: { transactionID },
